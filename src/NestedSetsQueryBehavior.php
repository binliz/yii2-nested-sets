...
class NestedSetsQueryBehavior extends Behavior
{
    ...
    public function isTreeCorrect()
    {
        /** @var $modelClass ActiveRecord */
        $modelClass = $this->owner->modelClass;
    }

    public function GetIncorectNodesRightKey()
    {
        /** @var $modelClass ActiveRecord */
        $modelClass = $this->owner->modelClass;
        $model = new $modelClass;
        ...
        $this->owner->andWhere(
                $modelClass::getDb()->quoteColumnName($model->leftAttribute) . ' >=' .
                $modelClass::getDb()->quoteColumnName($model->rightAttribute)
        );
        return $this->owner;
    }
    
    public function isCorrectCount()
    {
        /** @var $modelClass ActiveRecord */
        $modelClass = $this->owner->modelClass;
        $model = new $modelClass;

        $this->owner->select(['cnt'=>'count(id)','left'=>'min('.$modelClass::getDb()->quoteColumnName($model->leftAttribute).')',
                                 'right'=>'max('.$modelClass::getDb()->quoteColumnName($model->rightAttribute).')']);
